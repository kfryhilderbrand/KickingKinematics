...
%%% All 3 active, Thigh and Shin active (unlikely), Thigh and foot active
%%% (unlikely), Shin and Foot active, Thigh active (unlikely), Shin active
%%% (unlikely), Foot active
...
%%%variance explained exceeds 

%% Ankle Flexion
j3_act = JointAxis1Sens(iterations, init, GyroFootR, 1, CalAxes.Foot.AFlex);
%j3_act = -j3_act;
[J3_pca,~,~,~,explained3,~] = pca(GyroFootR(:,2:4));
j3_pca = J3_pca(:,1);
error3_EM = norm(j3_act-CalAxes.Foot.AFlex');%/norm(CalAxes.Foot.AFlex);
angle3_EM = atan2d(norm(cross(j3_act,CalAxes.Foot.AFlex')), dot(j3_act,CalAxes.Foot.AFlex'));
...
hold off
%% Ankle Inversion
j3_act = JointAxis1Sens(iterations, init, GyroFootR, 0, CalAxes.Foot.AInv);
%j3_act = -j3_act;
[J3_pca,~,~,~,explained3,~] = pca(GyroFootR(:,2:4));
j3_pca = J3_pca(:,1);
error3_EM = norm(j3_act-CalAxes.Foot.AInv');%/norm(CalAxes.Foot.AInv);
angle3_EM = atan2d(norm(cross(j3_act,CalAxes.Foot.AInv')), dot(j3_act,CalAxes.Foot.AInv'));
